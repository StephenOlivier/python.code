...
# ensure if it is at a turning point in a grid-based game for vertical movements
...
# ensure if it is at a turning point in a grid-based game for horizontal movements
	def isAtTurningCTR(self, row, col):
		...
		if (self.direction[0] == 1 or self.direction[0] == -1):
			if self.location[3] % self.cellDim == 0:
				if col + 1 >= self.cols:
					return self.board[int(row)][int(col) - 1] == 1
				elif col - 1 < 0:
					return self.board[int(row)][int(col) + 1] == 1
				else:
